...
const computeGeometry = ({ a, b, ee, f, r }) => {
  const lines = [];
  ...
  // Legacy drawing highlights the clean-out rectangle with a secondary pen.
  for (let index = 0; index < radiusRect.length; index += 1) {
    const start = radiusRect[index];
    const end = radiusRect[(index + 1) % radiusRect.length];
    addLine(start, end, 'secondary');
  }
...
function TechnicalDrawingQBFa({ a, b, ee, f, r }) {
  ...
  if (aVal <= 0 || bVal <= 0) {
    return (
      <div className="technical-drawing-empty" role="note">
        Brak danych do wygenerowania rysunku dla kolana symetrycznego QBFa.
      </div>
    );
  }

  const geometry = computeGeometry({ a: aVal, b: bVal, ee: eeVal, f: fVal, r: rVal });
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Rysunek techniczny kolana symetrycznego QBFa"
    >
      <title>Kolano symetryczne QBFa – widoki</title>
...
export default TechnicalDrawingQBFa;
