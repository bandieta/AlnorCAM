...
const ShapeGeometries = {
  ...
  QBFRa: (dims) => {
    ...
    // Normalize exactly as in Form1.cs
    ...
    // Points 21-24: Bottom intermediate (for arc connection)
    ...
    // Points 35-39: Top arc (90° in 5 segments)
    for (let i = 0; i < 5; i++) {
      const angle = (75 - 15 * i) * Math.PI / 180.0;
      const x = points[12][0] + Math.cos(angle) * nr;
      const y = points[12][1] + nr - Math.sin(angle) * nr;
      const idx = 39 - i; // Match Form1.cs: 39->75°, 35->15°
      setPoint(idx, x, y, na / 2);
    }

    const faces = [
      // Main section faces (original C# order)
      [4, 7, 6, 5],
      [5, 6, 2, 1],
      [4, 15, 12, 16],
      [8, 9, 1, 0],
      ...
      // Bottom arc quads (connecting straight sections to arc)
      [15, 21, 29, 12],
      [21, 22, 28, 29],
      [22, 5, 27, 28],
      [5, 23, 26, 27],
      ...
      // Arc connection quads (bottom to top)
      [12, 29, 39, 13],
      [29, 28, 38, 39],
      [28, 27, 37, 38],
      [27, 26, 36, 37],
